...
class PaytmChecksum {
    static encrypt(input, key) {
        if (!input || typeof input !== "string") {
            console.error("Invalid input for encryption:", input);
            return null;
        }
        if (!key || typeof key !== "string") {
            console.error("Invalid key for encryption:", key);
            return null;
        }
    ...
    static decrypt(encrypted, key) {
        if (!encrypted || typeof encrypted !== "string") {
            console.error("Invalid encrypted value:", encrypted);
            return null;
        }
        if (!key || typeof key !== "string") {
            console.error("Invalid key for decryption:", key);
            return null;
        }
    ...
    static async generateSignatureByString(params, key) {
        try {
            ...
            console.error("Error generating signature by string:", error);
            return null;
        }
    }
    ...
    static verifySignatureByString(params, key, checksum) {
        ...
        var paytm_hash = PaytmChecksum.decrypt(checksum, key);
        if (!paytm_hash) {
            console.error("Decryption failed! Invalid checksum or key.");
            return false;
        }
